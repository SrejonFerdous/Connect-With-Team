        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($name) && !empty($email) && !empty($pass) && !empty($salary) ){
                ...
                if( mysqli_num_rows($r) > 0 ){
                    ...
                    $sql = "UPDATE employee SET name = '$name' ,Phone = '$Phone',Department = '$Department',Role = '$Role', email = '$email', password ='$pass' , dob='$dob', gender='$gender' , salary='$salary' WHERE id = $_GET[id] ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                        echo "<script>
